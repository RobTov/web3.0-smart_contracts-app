...
const TransactionCard = ({ addressTo, addressFrom, message, keyword, amount, url, timestamp }) => {
        ...
        gradient-bg-transactions">
            <div className="flex flex-col md:p-12 py-12 px-4">
                {currentAccount ? (
                    ...
                    my-2">Latest Transactions</h3>)
                    : (
                        <h3 className="text-white text-3xl text-center
                    my-2">Connect your account to se latest transactions</h3>
                    )
                }

                <div className="flex flex-wrap justify-center items-center mt-10">
                    {transactions.reverse().map((transaction, index) => (
                        <TransactionCard key={index}  {...transaction} url="https://www.omnisend.com/blog/wp-content/uploads/2016/09/funny-gifs-5.gif" />
                    ))}
                </div>
            </div>
        </div>
